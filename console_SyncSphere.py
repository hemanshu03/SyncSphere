from flask import Flask, render_template, request, url_for, redirect, jsonify, send_from_directory, send_file, session, make_response
from flask_socketio import SocketIO, join_room, leave_room, send
from s_funcs import functions, EmailFunctions as eaf
import os
...
# Decrypt data
def decrypt_with_fernet(encrypted_data):
    global server_secret_key
    fernet = Fernet(server_secret_key)
    decrypted_data = fernet.decrypt(encrypted_data).decode()
    return decrypted_data

def caesar_cipher_decrypt_with_secret(text):
    shift = 4
    secret_phrase = "Lol this is the passphrase"
    decrypted_text = ""
    secret_index = 0
    for char in text:
        if char.isalpha():
            shift_amount = (ord(secret_phrase[secret_index % len(secret_phrase)]) - ord('A')) + shift
            if char.islower():
...
def chat_room():
    ...
    return render_template('test_session.html', rooms=rooms_)
...
@socketio.on('store_fingerprint')
def store_fingerprint(data):
    sid = data.get('sid')
    usm = data.get('usm')
    fingerprint = data.get('finprt')
    final_fnr = fingerprint + sid
    final_fnr = hashlib.sha256(final_fnr.encode()).hexdigest()
    print(f'=====================================================\nThe hashed stuff.. : {final_fnr}\n===================================================\n')
    session_user_dets[final_fnr] = {'username': usm}
...
def refresh_captcha():
    ...
    session_user_dets[session['session_id']] = {'captcha_text':captcha_text}
    return jsonify({'new_captcha': captcha_image})

@app.route('/login', methods=['POST', 'GET'])
def login(): #This function renders the login webpage.
    key = so.create_session_id()
    session['session_id'] = key
    session['session_type'] = 'register'
    session['logged_in'] = False
    cptch_img, cptch_txt = cptch.get_captcha_image()
    session_user_dets[key] = {'captcha_text':cptch_txt}
    return render_template('login.html', captcha_image=cptch_img, session_id=key) 

@app.route('/register')
def register__(): #This function renders the registration webpage (not yet ready).
    # Flask server-side session clear
    ...
    cptch_img, cptch_txt = cptch.get_captcha_image()
    session_user_dets[key] = {'captcha_text':cptch_txt}
    return render_template('create.html', captcha_image=cptch_img)
...
def send_otp(data):
    eaf.send_otp(email=data.get('email'), session_id=session['session_id'], ip=request.remote_addr)

@socketio.on('register_user')
def final_register_step(data):
    captcha_r = data.get('captcha_')
    sid = session['session_id']
    captcha_m = session_user_dets[sid]['captcha_text']
    email = data.get('eml')
    usernm = data.get('usmr')
...
def check_entered_details():
    ...
    if authenticate_(sessionId):
        ...
        if uo.check_user_details(data=dts) == 2 and captcha_r == session_user_dets[sessionId]['captcha_text']:
            session_user_dets[sessionId] = userid
            return jsonify({'status': 'success', 'url': '/rooms'})
        if captcha_r != session_user_dets[sessionId]['captcha_text']:
...
def write_config(data):
    """Writes configuration data to a configuration file."""
    config = configparser.ConfigParser()
    config['Server_Settings'] = data
...
def read_config():
    ...
    if 'Server_Settings' in config:
        settings = dict(config['Server_Settings'])

    return settings

def start_server(ch):
    """Starts the server based on the user's choice."""
    settings = read_config()
    
    app.config['SECRET_KEY'] = settings.get('sk', '')  # Replace with your secret key
    app.config['SESSION_TYPE'] = settings.get('st', '')  # Specifies the session type
    app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(minutes=int(settings.get('pslf', 30)))
    app.config['SESSION_COOKIE_SAMESITE'] = settings.get('scss', 'Strict')  # or 'Strict' or 'None'
...
if __name__ == '__main__':
    ...
    while choice not in [1, 2, 3]:
        clear_screen()
        choice = int(input("Choose your instruction:\n1. Start the server with default parameters\n2. Change the database connection URI\n3. Change the session timeout setting\nEnter the menu number as your choice: "))
    ...
    if choice == 1:
        ...
        ch = 0
        while ch not in [1, 2]:
            ch = int(input("Choose your instruction:\n1. Start on localhost\n2. Start on Public IP\nEnter the menu number as your choice: "))
            if ch not in [1, 2]:
                clear_screen()
                print("Please enter a valid choice.")
        start_server(ch)